...
export type WeekTime = RouterOutput['getScheduleDefaultWeekTimes']['scheduleDefaultWeekTimes'][0];
...
export function generateDaysWithTimes({
  ...
  const weekWithDays = week.map(day => {
    return {
      day,
      dates: weekdays.filter(weekday => weekday.weekDay === day).map(weekday => weekday.date),
    };
  });
