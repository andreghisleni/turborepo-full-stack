...
export const createDebugLogger =
  (name: string, formatter: (val: LogEntry) => string) => (): Logger => {
    ...
    return {
      ...
      commit: () => {
        if (isEnabled) {
          console.log(debugLogHeader(name)); // eslint-disable-line no-console
          ...
          console.log(debugLogFooter(name)); // eslint-disable-line no-console
        }
      },
    };
  };
...
function debugLogHeader(name: string) {
  return `[auth debug start: ${name}]`;
}

function debugLogFooter(name: string) {
  return `[auth debug end: ${name}] (@clerk/nextjs=${'0.0.0.0.'},`;
}
