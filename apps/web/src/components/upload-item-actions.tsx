...
export function UploadItemActions({ videoId }: UploadItemActionsProps) {
  ...
  const { mutateAsync: deleteVideo, isPending: isDeletingVideo } = trpc.deleteUpload.useMutation({
    onSuccess() {
    ...
    <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant="outline" size="xs" className="data-[state=open]:bg-muted">
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription className="space-y-3">
            <p>
              This action can&apos;t be undone and the video will be permanently deleted from the
              server.
            </p>
            <p>
              This will <span className="font-semibold text-accent-foreground">permanently</span>:
            </p>
        ...
        <AlertDialogFooter>
          ...
          >
            {isDeletingVideo ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Delete'}
          </Button>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
